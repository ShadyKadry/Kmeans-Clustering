...
@testset "kmeanspp_init" begin
    ...
    idxs1 = KMeansClustering.kmeanspp_init(X, k; rng = rng1)
    idxs2 = KMeansClustering.kmeanspp_init(X, k; rng = rng2)
    ...
    Xdeg = zeros(2, 10)
    idxs3 = KMeansClustering.kmeanspp_init(Xdeg, 5; rng = MersenneTwister(42))
    @test length(unique(idxs3)) == 5

    # Edge case: k = 1 (single center)
    rng_single = MersenneTwister(7)
    idxs_single = KMeansClustering.kmeanspp_init(X, 1; rng = rng_single)
    ...
    idxs_all = KMeansClustering.kmeanspp_init(X, ncols; rng = rng_all)
    ...
    @test_throws ArgumentError KMeansClustering.kmeanspp_init(X, 0; rng = MersenneTwister(9))
    @test_throws ArgumentError KMeansClustering.kmeanspp_init(X, ncols + 1; rng = MersenneTwister(10))
end
